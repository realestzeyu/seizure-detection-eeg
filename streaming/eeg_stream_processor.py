...
eeg_raw_df = (
    ...
    .option("startingOffsets", "earliest")  # IMPT: SET TO latest if prod. this line means to start from the earliest message in the topic, to not miss any data.
...
eeg_analytic_df = (
    eeg_analytic_df.withColumn("alert_reason", when(col("spike_count")>10, "spike_threshold").when(col("stddev_uv")>0.00001,"high_variance").otherwise(None))
    )
...
def process_batch(batch_df, batch_id):
    ...
    (
        ...
        .write.format("delta")
        .mode("append")
        .partitionBy("patient_id", "event_date") # saves data in folders of patient_id=xxx/event_date=xxx for efficient querying later
...
eeg_analytic_df.writeStream.foreachBatch(process_batch).option("checkpointLocation","./data/checkpoints/eeg_stream").start().awaitTermination()
